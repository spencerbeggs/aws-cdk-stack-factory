...
async function main(): Promise<void> {
	try {
		...
		if (APP_ENV) {
			const stackConfigFileExists = await fs.pathExists(path.resolve(__dirname, `../envs/${STACK}/${APP_ENV}.json`));
			if (!stackConfigFileExists) {
				throwErr(`No stack config file found at: envs/${STACK}/${APP_ENV}.json`);
			}
		} else {
			const defaultStackConfigFileExists = await fs.pathExists(path.resolve(__dirname, `../envs/${STACK}/default.json`));
